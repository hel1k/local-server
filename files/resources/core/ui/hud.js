...
A.on('ui:open', (screen, arg) => {
    ...
    else if (screen === 'debug') openBoard();
...
function openChat(prefill) {
    // чат может открыться поверх меню/доски (cmd-панель) — убираем их слои
    if (boardOpen) {
        boardOpen = false;
        A.emit('ui:cursor', false);
    }
...
function mainScreen() {
    return {
        ...
        items: [
            ...
            { sep: true },
            { label: 'Clothes debug', board: true },
            { sep: true },
...
function playerScreen() {
    return {
        ...
        items: [
            ...
            { sep: true },
            { label: 'Gender: male', run: () => act('gender', 'm'), stay: true },
            { label: 'Gender: female', run: () => act('gender', 'f'), stay: true },
            { sep: true },
