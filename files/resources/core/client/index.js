...
import { TATTOO_CATALOG } from './tattoocatalog.js';
...
function applyFace() {
    ...
    // причёску не возвращаем, пока её намеренно спрятали в браузере подложек
    if (!tatHairHidden) {
...
view.on('face:apply', (s) => {
    ...
    faceState = s;
    applyFace();
    // тинт красит декали только в момент наложения — перекладываем их сразу,
    // иначе цвет подложки менялся бы лишь после перелистывания
    if (tattooList.length || tattooPreview) applyTattoos();
    scheduleOutfitSave();
});

// ---------------- Тату и hair-подложки (декорации педа) ----------------
// каталог: ваниль из tattoocatalog.js + кастом-паки с сервера (tattoo:catalogExtra).
// Категории: зоны тела + HAIR (подложки под причёски, h=1). Фильтр по полу педа.

let tattooExtra = [];      // записи из кастом-паков
let tattooList = [];       // надетые: [{c, n}]
let tattooPreview = null;  // текущий кандидат при листании (не сохраняется)
const tattooCursor = {};   // категория -> индекс листания

alt.onServer('tattoo:catalogExtra', (rows) => {
    if (Array.isArray(rows)) tattooExtra = rows;
});

function tattooPool(cat) {
    const g = currentGender();
    const all = TATTOO_CATALOG.concat(tattooExtra);
    return all.filter((e) => (e.g === 'u' || e.g === g)
        && (cat === 'HAIR' ? e.h === 1 : e.h !== 1 && e.z === cat));
}

function applyTattoos() {
    const p = ped();
    const tint = () => {
        // декали с usesTintColor красятся палитрой волос: тинт должен быть выставлен
        // и ДО наложения (иначе нечего семплить), и после (переживает clear)
        try { native.setPedHairTint(p, Number(faceState.hairColor) || 0, Number(faceState.hairHighlight) || 0); }
        catch (e) { try { native.setPedHairColor(p, Number(faceState.hairColor) || 0, Number(faceState.hairHighlight) || 0); } catch (e2) { /* пусто */ } }
    };
    tint();
    try { native.clearPedDecorations(p); } catch (e) { /* пусто */ }
    for (const t of tattooList.concat(tattooPreview ? [tattooPreview] : [])) {
        const col = alt.hash(t.c);
        const ov = alt.hash(t.n);
        try { native.addPedDecorationFromHashes(p, col, ov); } catch (e) { /* пусто */ }
    }
    tint();
}

view.on('tattoo:nav', (cat, dir) => {
    const pool = tattooPool(cat);
    if (!pool.length) {
        tattooPreview = null;
        applyTattoos();
        view.emit('tattoo:info', { cat, name: '—', idx: -1, count: 0, applied: tattooList.length });
        return;
    }
    const cur = tattooCursor[cat] ?? -1;
    let idx = cur < 0 ? (Number(dir) < 0 ? pool.length - 1 : 0) : cur + Number(dir);
    if (idx < 0) idx = pool.length - 1;
    if (idx >= pool.length) idx = 0;
    tattooCursor[cat] = idx;
    tattooPreview = { c: pool[idx].c, n: pool[idx].n };
    applyTattoos();
    view.emit('tattoo:info', { cat, name: pool[idx].n, idx, count: pool.length, applied: tattooList.length });
});

view.on('tattoo:add', () => {
    if (!tattooPreview) return;
    tattooList.push(tattooPreview);
    tattooPreview = null;
    applyTattoos();
    scheduleOutfitSave();
    view.emit('tattoo:applied', tattooList.length);
});

view.on('tattoo:removeLast', () => {
    tattooList.pop();
    applyTattoos();
    scheduleOutfitSave();
    view.emit('tattoo:applied', tattooList.length);
...
view.on('tattoo:clear', () => {
    tattooList = [];
    tattooPreview = null;
    applyTattoos();
    scheduleOutfitSave();
    view.emit('tattoo:applied', 0);
});

// ---------------- Раздевание на время меню тату ----------------
// пока листаешь тату — пед голый (тату видно по всему телу), в категории
// подложек ещё и лысый; при выходе из меню всё возвращается как было

let tatSavedComps = null;
let tatHairHidden = false; // причёска снята тумблером в браузере подложек

function tatUndress() {
    if (tatSavedComps) return;
    const p = ped();
    tatSavedComps = [];
    for (let c = 0; c <= 11; c++) {
        try {
            tatSavedComps.push([native.getPedDrawableVariation(p, c), native.getPedTextureVariation(p, c)]);
        } catch (e) {
            tatSavedComps.push([0, 0]);
        }
    }
    const naked = currentGender() === 'f'
        ? { 1: 0, 3: 15, 4: 15, 5: 0, 6: 35, 7: 0, 8: 14, 9: 0, 10: 0, 11: 18 }
        : { 1: 0, 3: 15, 4: 14, 5: 0, 6: 34, 7: 0, 8: 15, 9: 0, 10: 0, 11: 15 };
    for (const c of Object.keys(naked)) {
        try { native.setPedComponentVariation(p, Number(c), naked[c], 0, 2); } catch (e) { /* пусто */ }
    }
    applyTattoos(); // после смены компонентов декорации перерисовываем
}
...
function tatRedress() {
    ...
    const p = ped();
    for (let c = 0; c <= 11; c++) {
        try { native.setPedComponentVariation(p, c, tatSavedComps[c][0], tatSavedComps[c][1], 2); } catch (e) { /* пусто */ }
    }
    tatSavedComps = null;
    applyTattoos();
}

// подложки видно только без причёски: в категории Hair base лысим, в остальных возвращаем
function tatHair(bald) {
    const p = ped();
    tatHairHidden = Boolean(bald);
    if (bald) {
        try { native.setPedComponentVariation(p, 2, 0, 0, 2); } catch (e) { /* пусто */ }
    } else if (tatSavedComps) {
        try { native.setPedComponentVariation(p, 2, tatSavedComps[2][0], tatSavedComps[2][1], 2); } catch (e) { /* пусто */ }
    } else {
        try { native.setPedComponentVariation(p, 2, Number(faceState.hair) || 0, 0, 2); } catch (e) { /* пусто */ }
    }
}

view.on('tattoo:menu', () => tatUndress());

// тумблер причёски: подложка живёт под волосами — иногда нужно глянуть с ними,
// иногда без (проверить саму подложку и как её красит тинт)
view.on('tattoo:hair', (show) => {
    tatHair(!show);
    applyTattoos();
});

// выход из экрана тату: скинуть незакреплённый предпросмотр и одеться обратно
view.on('tattoo:closePreview', () => {
    tattooPreview = null;
    if (tatSavedComps) tatRedress();
    else applyTattoos();
});
...
function faceCamUpdate() {
    ...
    // и колесо тоже пробуем нативно (id и группа контрола гуляют между билдами);
    // основной путь — событие makeup:zoom из WebView, у которого фокус
    const wheel = (ids) => ids.some((c) => [0, 1, 2].some((g) => native.isDisabledControlJustPressed(g, c)));
    if (wheel([241, 14, 16])) faceOrbit.dist = Math.max(faceOrbit.dist - 0.07, 0.22);
    if (wheel([242, 15, 17])) faceOrbit.dist = Math.min(faceOrbit.dist + 0.07, 2.5);
...
// зум колесом приходит из WebView — фокус во время студии у него
view.on('makeup:zoom', (dir) => {
    if (faceCam === null) return;
    const d = Number(dir) > 0 ? 0.07 : -0.07;
    faceOrbit.dist = Math.min(Math.max(faceOrbit.dist + d, 0.22), 2.5);
});
...
alt.onServer('creator:open', () => openUI('creator'));
...
function collectOutfit() {
    ...
    return { comps, props, face: faceState, tattoos: tattooList, gender: currentGender() };
}
...
alt.onServer('outfit:apply', (outfit) => {
    ...
    tattooList = Array.isArray(outfit.tattoos) ? outfit.tattoos : [];
    tattooPreview = null;
    applyTattoos();
});
